...
function mascot() {}
const PREFIX = "trusted-swarm-room-";
...
async function getWeightCache() {
  ...
  try { weightCache = await caches.open("trusted-swarm-weights-v1"); } catch { weightCache = false; }
  return weightCache;
}
function cacheKey(url, lo, hi) { return "https://weights.trustedswarm.local/" + encodeURIComponent(url) + "/" + lo + "-" + hi; }
...
$("cache-clear").addEventListener("click", async (ev) => {
  ev.preventDefault();
  try { await caches.delete("trusted-swarm-weights-v1"); weightCache = null; toast("cached weights cleared"); } catch { toast("could not clear the cache"); }
});
function aiSubmit() {
